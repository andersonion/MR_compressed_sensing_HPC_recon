...
#
declare -x CS_CODE_DEV=iter_strat_v2;
if [ -z "$1" ];then
    concurrent_vols=5;
else
    concurrent_vols=$1;
...
if [ $in_progress_count -lt $concurrent_vols ]; then
	...
	touch $tf
	streaming_CS_recon kamy S67962 LOCAL FID first_volume=$nv last_volume=$nv iteration_strategy=10x5 skip_fermi_filter planned_ok chunk_size=5 CS_reservation=jjc29_33;
    fi;
else
