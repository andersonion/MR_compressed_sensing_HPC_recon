...
if ~exist('USE_OLD_REMOTE_CHECK_CODE','var')
    cmd_stack={};
    ssh_dest=sprintf('%s@%s',sys_user(),  target_host_name);
...
ship_prep_send_procpar = sprintf( [ ...
    ...
    sys_user(),  target_host_name,  options.target_machine,  setup_var.volume_runno,...
    options.target_machine,  setup_var.volume_runno,...
    recon_mat.procpar_file,  sys_user(),  target_host_name,  options.target_machine,  setup_var.volume_runno);
